...
public struct AnyProduct: Identifiable, Codable, Sendable {
    ...
    public static let mockYearly: AnyProduct = .init(
    ...
    public static let mockMonthly: AnyProduct = .init(
        id: "mock.monthly.id",
...
public extension Array where Element == AnyProduct {
    var eventParameters: [String: Any] {
        var dict: [String: Any?] = [
            "products_count": count,
            "products_ids": compactMap { $0.id }.sorted().joined(separator: ", "),
            "products_titles": compactMap { $0.title }.sorted().joined(separator: ", ")
        ]
        for product in self {
