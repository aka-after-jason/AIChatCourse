    ...
    var body: some View {
        List {
            ...
            if !presenter.popularAvatars.isEmpty {
                if presenter.categroyRowTest == .top {
                    categorySection
                }
            }
            ...
            if !presenter.popularAvatars.isEmpty {
                if presenter.categroyRowTest == .original {
                    categorySection
                }
                popularSection
            }
        ...
        .task {
            await presenter.loadFeaturedAvatars() // 没有先后顺序
        }
        .task {
            await presenter.loadPopularAvatars() // 没有先后顺序
        }
        .task {
